	...
	try {
		...
		method.apply( _this , args.concat(function(){
			...
			if ( replyTo && correlationId ) {
				...
				_this._sendReply( replyTo , correlationId , _.toArray(arguments) ).then( ack , ack );
		...
		_this._sendReply( replyTo , correlationId , [ err ] ).then( ack , ack ) ;
...
RPC.prototype._queueName = function () {
	
	return format('rpc.%s',uuid.v4()) ;
