	...
	return self._auxChannelPromise.then(function(ch){ 
		return when.promise(function(resolve,reject){
			...
			if( ok ){
				...
				ch.once('drain',function(){ resolve(true) })
				ch.once('error',function(err){ reject(err) })
			}
		})
	});
...
Connection.prototype.sendToQueue = function ( queueOrName , message , options , callback ) {
	...
	return self._auxChannelPromise.then(function(ch){ 
		...
		ch.once('drain',function(){ def.resolve(true) })
		ch.once('error',function(err){ def.reject(err) })
