...
function Connection ( amqpOpts , sockOpts ) {
	...
	var self = this ;
	var debug = self._debug = (amqpOpts && amqpOpts.debug)  || (sockOpts && sockOpts.debug);
...
Connection.prototype._open = function () {
	...
	var promise = recant.promise(function(resolve,reject,notify,reset){
		...
		re.try(function(retryCount, done){
			...
			self.emit('reconnecting', format( 'connecting attempt [%s]' , retryCount+1 ) )
			...
			if ( err ) {
				self.emit('error', format('exhausted recconnection after %s attempts' , retryCount ) ) ;
				reject(err) ;
			} else {
				self.emit('ready', format('connected after %s attempts',retryCount) ) ;
